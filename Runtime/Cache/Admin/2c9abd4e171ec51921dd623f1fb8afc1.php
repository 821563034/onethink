...
<body>
    ...
    <div class="main-title">
        <h2><?php echo isset($info['id'])?'编辑':'新增';?>任务</h2>
    </div>
    <form action="<?php echo U();?>" method="post" class="form-horizontal">
        ...
        <div class="form-item">
            ...
            <div class="controls">
                <input type="text" class="text" id="start_position_text" value="<?php echo ((isset($start_depart) && ($start_depart !== ""))?($start_depart):'请选择位置'); ?>" onclick="open_iframe(1);">
                <input type="hidden" class="text" id="start_position" name="start_position" value="<?php echo ((isset($info["start_position"]) && ($info["start_position"] !== ""))?($info["start_position"]):'0'); ?>">
            </div>
        </div>
        <div class="form-item">
            <label class="item-label">目标位置<span class="check-tips">（任务目标点，可以是多个）</span></label>
            <div class="controls">
                <input type="text" class="text" id="end_position_text" value="<?php echo ((isset($end_depart) && ($end_depart !== ""))?($end_depart):'请选择位置'); ?>" onclick="open_iframe(2);">
                <input type="hidden" class="text" id="end_position" name="end_position" value="<?php echo ((isset($info["end_position"]) && ($info["end_position"] !== ""))?($info["end_position"]):'0'); ?>">
            </div>
        </div>
        <div class="form-item">
            ...
            <div class="controls">
                <select name="dept" id="dept" onchange="ajax_dept();">
                    <?php if(is_array($dept)): $i = 0; $__LIST__ = $dept;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><option value="<?php echo ($v["id"]); ?>"><?php echo ($v["title_show"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                </select>
            </div>
        </div>
        <div class="form-item">
    ...
    <script type="text/javascript">
        Think.setValue("type", <?php echo ((isset($info["pid"]) && ($info["pid"] !== ""))?($info["pid"]): 0); ?>);
        Think.setValue("uid", <?php echo ((isset($info["dept"]) && ($info["dept"] !== ""))?($info["dept"]): 0); ?>);
        //导航高亮
        ...
        function ajax_dept(){
            $.post('<?php echo U("ajax");?>',{id:$('#dept').val()},function (result) {
                console.log(result);
                if(result){
